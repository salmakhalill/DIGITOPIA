...
const Home = () => {
  ...
  const [reportsChart, setReportsChart] = useState(null);
  const [crimePieChart, setCrimePieChart] = useState(null);
 ...
 useEffect(() => {
  ...
  fetch(`https://salmakhalill.pythonanywhere.com/api/analytics/recent/?period=${period}`, {
    method: "GET",
    headers: {
  ...
  useEffect(() => {
    if (!data?.charts?.bar_chart) return;

    const labels = Object.keys(data.charts.bar_chart);
    const values = Object.values(data.charts.bar_chart);
  ...
  useEffect(() => {
    ...
    const labels = Object.keys(data.charts.status_distribution);
    const values = Object.values(data.charts.status_distribution);
    const ctx = document.getElementById("crimePieChart");
    if (!ctx) return;
    if (Chart.getChart(ctx)) Chart.getChart(ctx).destroy();
    ...
    <div className="wrapperr mt-5 mb-5">
      ...
      <div className="dashboard-content row align-items-center">
        <div className="info col-12 col-lg-6">
          <h1 className="fw-bold">لوحة التحكم</h1>
  ...
  <div className="reborts mt-2 row g-3">
        {data?.kpis &&
          Object.entries(data.kpis).map(([, item], i) => {
            const customLabels = [
              "البلاغات الكلية",
              "البلاغات الجديدة",
              "البلاغات تحت المراجعة",
              "البلاغات الحرجة",
            ];

            let periodNote = "";
            if (i === 0 || i === 2) {
              periodNote = "آخر 4 شهور";
            } else if (i === 1 || i === 3) {
              periodNote = "آخر يوم";
            }
