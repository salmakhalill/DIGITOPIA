...
const Statistics = () => {
  ...
  const getThemeColors = useCallback(() => {
    return {
      fontColor: getVar("--font-color"),
      mainColor: getVar("--main-color"),
      purple: getVar("--purple"),
      darkPurple: getVar("--dark-purple"),
    };
  }, [getVar]);

  const updateChartsColors = useCallback(() => {
    const { fontColor, mainColor } = getThemeColors();
    ["reportsChart", "crimePieChart", "barChart"].forEach((id) => {
      const chart = Chart.getChart(document.getElementById(id));
      if (!chart) return;
      chart.options.plugins.legend.labels.color = fontColor;
      chart.options.plugins.tooltip.backgroundColor = mainColor;
      chart.options.plugins.tooltip.titleColor = fontColor;
  ...
  const loadData = useCallback(async () => {
    try {
      const json = await getDashboardData(year, location);
      setData(json);
    } catch (err) {
      console.error("Error loading data:", err);
    }
  }, [year, location, getDashboardData]);
  ...
  useEffect(() => {
    if (!data) return;
    let { fontColor, mainColor, purple, darkPurple } = getThemeColors();
    ...
    if (lineCtx) {
      ...
      new Chart(lineCtx, {
        type: "line",
        data: {
          labels: monthly.map(([month]) => month),
          datasets: [
            {
              label: "عدد البلاغات",
              data: monthly.map(([, v]) => v),
              borderColor: purple,
              backgroundColor: "rgba(70, 47, 138, 0.2)",
              fill: true,
              tension: 0.3,
            },
          ],
    ...
    // Pie Chart
    const pieCtx = document.getElementById("crimePieChart");
    if (pieCtx) {
      if (Chart.getChart(pieCtx)) Chart.getChart(pieCtx).destroy();
      new Chart(pieCtx, {
        type: "doughnut",
        data: {
          labels: Object.keys(data.charts.case_status_distribution),
          datasets: [
            {
              data: Object.values(data.charts.case_status_distribution),
              backgroundColor: [
                purple,
                darkPurple,
                "#4CAF50",
    ...
    if (barCtx) {
      ...
      new Chart(barCtx, {
        type: "bar",
        data: {
          labels: Object.keys(data.charts.report_type_distribution),
          datasets: [
            {
        ...
        options: {
          ...
          scales: {
            x: { ticks: { color: fontColor } },
            y: { ticks: { color: fontColor, stepSize: 100 } },
          },
        },
      });
    }
  ...
  const convertLatLngToXY = (lat, lng) => {
    ...
    const maxLng = 36.894654;
    const svgWidth = 548.58221;
    const svgHeight = 498.86664;
  ...
  useEffect(() => {
    const handleResize = () => {
      ["reportsChart", "crimePieChart", "barChart"].forEach((id) => {
        ...
        if (ctx && Chart.getChart(ctx)) Chart.getChart(ctx).resize();
      });
    };
    ...
    return () => {
      ...
      window.removeEventListener("themeChanged", updateChartsColors);
    };
  }, [updateChartsColors]);

  if (!data)
    return (
      <p className="text-center text-[var(--font-color)] animate-pulse">
        جار التحميل...
      </p>
    );
    ...
    <div className="wrapperr">
      ...
      <div className="dashboard-content row align-items-center">
        <div className="info col-12 col-lg-6">
          <h1 className="fw-bold">الإحصائيات</h1>
      ...
      <div className="kpi-wrapper">
        {[
          ...
          {
            title: "أعلى نوع بلاغ",
            value: data.kpis.top_report_type,
            iconClass: "fas fa-folder-open",
          },
          {
            title: "نسبة الحل",
            value: `${data.kpis.solved_percentage.toFixed(2)}%`,
            iconClass: "fas fa-check-circle",
          },
          {
            title: "المحافظة الأعلى",
            value: data.kpis.top_region,
            iconClass: "fas fa-map-marker-alt",
          },
        ].map((item, i) => (
          <div key={i} className="kpi-col">
      ...
      <div className="dashboard-grid">
        ...
        {/* حالة البلاغات */}
        <div className="dashboard-card">
          <h3 className="text-end">حالة البلاغات</h3>
        ...
        <div className="dashboard-card">
          ...
          <div id="heatMap">
            ...
            >
              ...
              {Array.isArray(data?.charts?.heatmap) &&
                data.charts.heatmap.map((point, index) => {
                  const { x, y } = convertLatLngToXY(
                    point.latitude,
                    point.longitude
                  );
                  return (
